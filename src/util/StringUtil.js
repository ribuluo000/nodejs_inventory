...
function obj(){
    ...
    /**
     * 订单流水号生成 时间年月6位(171201) + 类型1位(1充值,2流水) + 支付方式1位(0流水,1微信,2支付宝) + 毫秒时间戳后4位 + 用户ID后4位 共16位
     */
    this.paySerialNumber = (parameter) => {
        let type = Number(parameter.type);
        let payMethod = Number(parameter.payMethod);
        ...
        let sRandom = String(nowTime).substr(String(nowTime).length - 4, 4);
        let sUserId = user_id.substr(user_id.length - 4, 4);

        return `${year}${month}${day}${type}${payMethod}${sRandom}${sUserId}`;
    };
